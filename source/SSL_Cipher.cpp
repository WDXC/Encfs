...
int BytesTokey(int keyLen, int ivLen, const EVP_MD* md,
  ...
  EMP_MD_CTX* cx = EVP_MD_CTX_new();
  EVP_MD_CTX_init(cx);

  for (;;) {
    EVP_DigestInit(cx, md, nullptr);
    if ((addmd++) != 0) {
...
SSL_Cipher::SSL_Cipher(const Interface& iface, const Interface& realIface_,

...
const ConfigVar& operaotr>>(const ConfigVar& src, Interface iface) {
  src >> iface.name();
  src >> iface.current();
  src >> iface.revision();
  src >> iface.age();

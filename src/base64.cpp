...
namespace encfs {
  ...
  // change between two power of two, stored as the low bits of the bytes in
  // the array.
  // It is the caller's responsiblity to make sure the output array is large
  // enough
  void changeBase2(unsigned char* src, int srcLength, int src2Pow,
      unsigned char* dst, int dstLength, int dst2Pow) {
    unsigned long work = 0;
    int workBits = 0;
    unsigned char* end = src + srcLength;
    ...
    if ((workBits != 0) && ((dst - origDst) < dstLength)) {
      *dst++ = work & mask;
      work >>= dst2Pow;
      workBits -= dst2Pow;
    }

    // now, we could have a paritial value left in the work buffer..
    if ((workBits != 0) && ((dst - origDst) < dstLength)) {
      *dst++ = work & mask;
    }
  }
  ...
  // output is assumed to be large enough toaccept the data.
  //
  ...
  void changeBase2Inline(unsigned char* src, int srcLength, int src2Pow,
      int dst2Pow, bool outputPartialLastByte) {
    changeBase2Inline(src, srcLength, src2Pow, dst2Pow, outputPartialLastByte, 0, 0, nullptr);
  }
  ...
  void AsciiToB32(unsigned char* out, const unsigned char* in,
      int length) {
    while ((length--) != 0) {
      unsigned char ch = *in++;
      int lch = toupper(ch);
...
static const unsigned char d[] = {
  ...
  bool B64StandardDecode(unsigned char* out, const unsigned char* in,
    ...
    while (in < end) {
      unsigned char v = *in++;
      if (v > 'z') {
        RLOG(ERROR) << "Invalid chracter: " << (unsigned int) v;
        return false;
      }
  ...
  // If you want to use an alternate alphabet, chage the characters here
  const static char encodeLookup[] =
  "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
  std::string B64StandardEncode(const std::vector<unsigned char>& inputBuffer) {
    std::string encodedString;
    encodedString.reserve(B256ToB32Bytes(inputBuffer.size()));
    long temp;
    ...
    switch (inputBuffer.size() % 3) {
      case 1:
        temp = (*cursor++) << 16;
        encodedString.append(1, encodeLookup[(temp & 0x00FC0000) >> 18]);
        encodedString.append(1, encodeLookup[(temp & 0x0003F000) >> 6]);
